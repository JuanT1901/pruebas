...
const obtenerInfoEscalaPrimaria = (notaStr: string | number) => {
  ...
  if (nota >= 1.0 && nota <= 3.4) {
    return { texto: 'Bajo', color: '#ffffff', bg: '#ef4444', icon: <FaThumbsUp size={16} color="white" /> } // Rojo
  }
  if (nota >= 3.5 && nota <= 3.9) {
    return { texto: 'Básico', color: '#ffffff', bg: '#22c55e', icon: <FaThumbsUp size={16} color="white" /> } // Verde
  }
  if (nota >= 4.0 && nota <= 4.4) {
    return { texto: 'Alto', color: '#ffffff', bg: '#3b82f6', icon: <FaThumbsUp size={16} color="white" /> } // Azul
  }
...
export default function PlanillaBasicaPage() {
  ...
  const guardarCompetenciaLocal = () => {
    ...
    const info = obtenerInfoEscalaPrimaria(notaNum)
  ...
  const infoEscalaActual = obtenerInfoEscalaPrimaria(tempNota)
      ...
      {vistaActual === 'lista' && (
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <header className={styles.header} style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '30px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }}>
              <h1 style={{ margin: 0 }}>Planilla Académica 📚</h1>
          ...
          {cargandoEvaluacion ? (
            ...
            <>
              <div className={styles.card} style={{ marginBottom: '30px', borderLeft: editandoIndex !== null ? '5px solid #f59e0b' : '5px solid #10b981', backgroundColor: editandoIndex !== null ? '#fffbeb' : 'white' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  <h3 style={{ margin: 0, color: editandoIndex !== null ? '#b45309' : '#1e293b', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    {editandoIndex !== null ? <><FaEdit /> Editando Desempeño</> : 'Añadir Desempeño y Nota'}
                  </h3>
                ...
                <div style={{ display: 'flex', gap: '15px', marginBottom: '15px', flexWrap: 'wrap' }}>
                  <div style={{ flex: '1 1 45%' }}>
                    ...
                    <textarea 
                      ...
                      placeholder="Ej: Comprende textos cortos..."
                  ...
                  <div style={{ flex: '1 1 45%' }}>
                    ...
                    <textarea 
                      ...
                      placeholder="Ej: Lee fluidamente y responde preguntas..."
                      style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', resize: 'none', overflow: 'hidden', minHeight: '42px', fontFamily: 'inherit', backgroundColor: 'white' }}
                    />
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '15px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: '1', display: 'flex', alignItems: 'center', height: '46px', backgroundColor: infoEscalaActual.bg, color: 'white', padding: '0 15px', borderRadius: '6px', fontWeight: 'bold', border: `1px solid ${infoEscalaActual.bg}`, gap: '10px', transition: 'background-color 0.3s' }}>
              ...
              <div style={{ marginBottom: '30px' }}>
                <h3 style={{ color: '#1e293b', marginBottom: '15px' }}>Desempeños Registrados</h3>
                
                {competencias.length === 0 ? (
                  <p style={{ color: '#64748b', textAlign: 'center', backgroundColor: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px dashed #cbd5e1' }}>
                    Aún no has agregado evaluaciones para esta materia.
                  </p>
                ) : (
                  <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    {competencias.map((item, idx) => {
                      const estiloGuardado = obtenerInfoEscalaPrimaria(item.nota || 0) 
                      ...
                      return (
                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: estaEditandoEste ? '#fef3c7' : 'white', padding: '15px', borderRadius: '8px', border: estaEditandoEste ? '2px dashed #f59e0b' : '1px solid #cbd5e1', marginBottom: '10px', opacity: estaEditandoEste ? 0.6 : 1, transition: 'all 0.2s' }}>
                          <div>
                            ...
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', marginTop: '10px', fontSize: '0.9rem', backgroundColor: estiloGuardado.bg, color: 'white', padding: '6px 12px', borderRadius: '6px', fontWeight: 'bold' }}>
                              {estiloGuardado.icon}
